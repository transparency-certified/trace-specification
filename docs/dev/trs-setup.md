...
The example assumes that you have a system running **Ubuntu 24.04 LTS**.  We assume that you can execute commands as the `root` user via `sudo`. We recommend that you create a dedicated user for running the TRACE Server. In our example we will use the username `tracesys`.
...
gpg: /home/tracesys/.gnupg/trustdb.gpg: trustdb created
gpg: directory '/home/tracesys/.gnupg/openpgp-revocs.d' created
gpg: revocation certificate stored as '/home/tracesys/.gnupg/openpgp-revocs.d/9A5209A464B9E0CFCCD108C96FB96730E419869C.rev'
...
### Initiate TRO before executing user code:

```bash
tro-utils --declaration sample_tro.jsonld \
    --profile ~/trs.jsonld \
...
```
Loading profile from /home/tracesys/trs.jsonld
```
...
```bash
tro-utils --declaration sample_tro.jsonld arrangement add sample-trace-workflow \
