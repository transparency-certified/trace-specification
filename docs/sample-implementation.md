...
- Having a way to sign the TROs (Trace Record Objects) that are generated by the TRACE server. TRACE allows for GPG and X.509. See [Signing](./dev/signing.md) for more information.
...
After [obtaining an API key](https://developers.zenodo.org/), we can use the provided [zenodo_upload.py](./dev/zenodo_upload.py) to create a draft deposit. Note that the script uses `sandbox.zenodo.org` by default:
