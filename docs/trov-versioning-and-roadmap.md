...
4. **Two perspectives, one vocabulary.** TROV data is simultaneously valid JSON and valid RDF. Producers and consumers alike may choose either perspective: some will treat TRO declarations as JSON documents validated against a schema; others will load them into a graph and query with SPARQL. Both perspectives must remain valid across versions. A version change that serves one perspective at the expense of the other is considered a design flaw. (See [TRO Declaration Design](tro-declaration-design.md) for the design principles underlying this dual perspective.)
